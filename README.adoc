...
The jar is containing two application - a webservice that produces data, and filtering processor (based on SpringCloud Streams and Kafka Streams).
They use separate runners.
An https://docs.cloudfoundry.org/devguide/multiple-processes.html[CF experimental API] should be used.
There are two commands defined in In `Procfile`.
